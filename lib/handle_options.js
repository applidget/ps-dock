...
var Handler = function(args){
  ...
  args = hash.newArgs;
  console.log(handler.options.env);
  console.log(args);
  this.settings = this.searchConfigFiles(this.parseConfigFile, function(){
    ...
    handler.options.bindPort = handler.deleteOption("--bind-port", handler.optionsDefaultValue(handler, handler.options.bindPort == "true", false)),
...
var readEnvOptions = function(args) {
  ...
  for(i = 0; i < args.length; i++) {
    if (args[i] == "-D" || args[i].indexOf("-D") == 0) {
      ...
      evar = pair.split(/=(.+)?/); //split only after the first '=' sign
