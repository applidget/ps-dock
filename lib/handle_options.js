...
var Handler = function(args){
  ...
  this.searchConfigFiles(function(properties, options){
  ...
  console.log(this.options.bindPort)
...
Handler.prototype.searchConfigFiles = function(callback, options) {
  ...
  try{
    ...
    if(stat.isFile()){
      Sync(function(){
        try{
          new Properties().load.sync (null, process.cwd() + '/.config', function(){
    ...
    try {
      ...
      if(stat.isFile()){
        ...
        properties.loadConfig = function(callback){
          properties.load (process.env["HOME"] + '/.config', callback);
        }
        var propertiesObject = properties.syncGetter('loadConfig');
        callback(propertiesObject, options);
      }
    }
    catch (e) {
