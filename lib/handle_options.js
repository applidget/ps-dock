...
var Handler = function(args){
  ...
  var path;
  var handler = this;
  this.settings = this.searchConfigFiles(this.options, this.parseConfigFile, function(){
    handler.options = {
      webHookUrl: handler.options.webHookUrl == undefined ? (uri == undefined ? undefined : (uri.protocol == undefined ? undefined : uri)) : handler.options.webHookUrl,
      bindPort: handler.deleteOption("--bind-port", handler.optionsDefaultValue(handler, handler.options.bindPort == "true", false)),
      dropUnhandledLogs: handler.deleteOption("--drop-unhandled-logs", handler.optionsDefaultValue(handler, handler.options.dropUnhandledLogs == "true", true)),
      logrotation: interpretRotation(handler.deleteOption("--log-rotate", handler.optionsDefaultValue(handler, handler.options.logrotation, "daily"))),
      stdout: handler.deleteOption("--stdout", handler.optionsDefaultValue(handler, handler.options.stdout, undefined)),
      psPort: handler.optionsDefaultValue(handler, parseInt(handler.options.psPort), process.env.PS_PORT || process.env.PORT),
      timeout: handler.deleteOption("--timeout", handler.optionsDefaultValue(handler, parseInt(handler.options.timeout), 30000)),
      command: handler.args.join(" ")
    }
    handler.options.logging = handler.parseStdout(handler.options.stdout);
    return 1;
  });
}
...
Handler.prototype.parseConfigFile = function(properties, options, handler, callback){
...
Handler.prototype.searchConfigFiles = function(options, callback, arg) {
  var handler = this;
  try{
    var dir = configPaths.pop();
    if (dir == undefined){
      util.log("No config files found !");
      return arg(handler);
    }
    stat = fs.lstatSync(dir + '/.psdockrc');
    if(stat.isFile()){
      new Properties ().load (dir + '/.psdockrc', function (){
          callback(this, options, handler, arg);
      });
      return 0;
    }
  }
  catch (e) {
    return handler.searchConfigFiles(options, callback, arg)
  }
};
