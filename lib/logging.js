...
Logger.prototype.drain = function() {
  ...
  var removeDrain =  function() {
    logger.removeListener('drain', drainCallback);
    logger.removeListener('enddrain', removeDrain)
  };
  logger.on('enddrain',removeDrain);
};
...
Logger.prototype.newStream = function(data, sync) {
  ...
  var rotateLogs = function(logContext, files) {
    ...
    var waitForLogOpen = function() {
      logger.removeAndCreateLogs();
      logger.removeListener('LogOpen', waitForLogOpen);
    }
    logger.on('LogOpen', waitForLogOpen);
...
Logger.prototype.listenStream = function (stream) {
  ...
  stream.on("data", function(data) {
    ...
    } else if (!logger.dropUnhandledLogs) {
      console.log("wahou");
      logger.buffer += data;
    }
  }); 
