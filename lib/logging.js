...
Logger.prototype.pathAndDate = function (fileName) {
  ...
  var pattern = new RegExp(firstPart + "\-[0-9]*.log$", 'g');
  var candidateFiles = _.filter(filesList, function(fileName) { return pattern.exec(fileName) != null });
  var existingFile = null;
  var logDate = logger.formattedddate();
  if (candidateFiles.length > 0) {
    existingFile = candidateFiles[0];
    var tokens = existingFile.split("-");
    var logDateAndLog = tokens[tokens.length - 1];
    tokens = logDateAndLog.split(".log");
    logDate = tokens[0];
  }
