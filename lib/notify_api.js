...
var notificator = function (url, timeout){
  ...
  if (this.uri != undefined){
    this.httpLib = require( (this.uri.protocol == "https:" || this.uri.protocol == "https") ? "https" : "http" );
...
notificator.prototype.requestHook = function(stat) {
  ...
  if(notificator.uri.query){
    optionsUpdate.path += '?';
    var concat = false;
    for (query in notificator.uri.query){
      optionsUpdate.path += (concat ? '&' : '') + query + '=' + notificator.uri.query[query];
      concat = true;
    }
  }
  else {
    optionsUpdate.path = notificator.uri.path;
  }
  ...
  hookRequest.on('error', function(e){
    switch (e.code){
      ...
      case 'ECONNREFUSED':
        ...
        break;
      default:
        util.log('Attempt to connect to the distant WEB HOOK finished with the following error code : ' + e);
        break;
    }
    clearTimeout(timeoutId)
    notificator.removeListener('webHookAnswered', not_timeout)
    notificator.updateNotificatorStatus('continue');
    return;
  });
