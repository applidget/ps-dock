...
TcpLogger.prototype.emitLoggerAvailable = function () {
  tcpLogger = this;
  tcpLogger.emit('loggerIsAvailable');
  tcpLogger.loggerIsNotAvailable = false;
}
...
TcpLogger.prototype.write = function (data){
  ...
  if(this.connected){
    if (this.logPrefix != null) {
      data = colors.addPrefixToLines(data, this.logPrefix, this.logPrefixColor);
    }
    tcpLogger.isWrittingData = true;
    this.sockRemoteShell.write(data, 'utf-8', function() {
      tcpLogger.isWrittingData = false;
      tcpLogger.emit('writteCompleted');
    }); 
  }
};
